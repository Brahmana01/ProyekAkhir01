...
<head>
    @include('admin.head')
    <title>Admin - Daftar Prestasi</title>
</head>
                    ...
                    <div class="card my-4">
                        <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                            <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
                                <h6 class="text-white text-capitalize ps-3">Daftar Prestasi</h6>
                            </div>
                        </div>
                                        ...
                                        <tbody>
                                            @foreach($prestasies as $prestasi)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <p class="text-xs font-weight-bold mb-0">{{ $prestasi->deskripsi }}</p>
                                                    </td>
                                                    <td>
                                                        <img src="{{ asset('storage/prestasi/' . $prestasi->foto) }}" alt="{{ $prestasi->judul }}" width="100">
                                                    </td>
                                                    <td>
                                                        <a href="{{ route('admin.prestasi.show', $prestasi->id) }}" class="btn btn-info btn-sm">Lihat</a>
                                                        <a href="{{ route('admin.prestasi.edit', $prestasi->id) }}" class="btn btn-warning btn-sm">Edit</a>
                                                        <form action="{{ route('admin.prestasi.destroy', $prestasi->id) }}" method="POST" style="display: inline;">
                                                            @csrf
                                                            @method('DELETE')
